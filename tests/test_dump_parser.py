...
class TestDumpParser:
    ...
    @pytest.mark.asyncio
    async def test_max_sample_docs_none_loads_all(self, mock_dump_dir: Path):
        """Test that max_sample_docs=None loads all documents."""
        dump_root = next(
            d for d in mock_dump_dir.iterdir() if d.name.startswith("dump-")
        )
        index_dir = dump_root / "indexes" / "products"

        # Create a file with some documents
        doc_count = 500
        with open(index_dir / "documents.jsonl", "w") as f:
        ...
        dump_file = mock_dump_dir / "all_docs.dump"
        with tarfile.open(dump_file, "w:gz") as tar:
            tar.add(dump_root, arcname=dump_root.name)

        # max_sample_docs=None should load all documents
        ...
        # Field distribution should be exact
        assert idx.stats.field_distribution["id"] == doc_count
        assert idx.stats.field_distribution["title"] == doc_count
    ...
    @pytest.mark.asyncio
    async def test_field_distribution_accurate_for_all_docs(self, mock_dump_dir: Path):
        """Test that field distribution counts all documents accurately."""
        dump_root = next(
            d for d in mock_dump_dir.iterdir() if d.name.startswith("dump-")
        )
        index_dir = dump_root / "indexes" / "products"

        # Create documents where some fields only appear in later documents
        total_docs = 200
        with open(index_dir / "documents.jsonl", "w") as f:
            for i in range(total_docs):
                # Add a field that only appears in the last 50 documents
                if i >= 150:
        ...
        dump_file = mock_dump_dir / "late_fields.dump"
        with tarfile.open(dump_file, "w:gz") as tar:
            tar.add(dump_root, arcname=dump_root.name)

        # Even with max_sample_docs=50, field distribution should count ALL documents
        parser = DumpParser(dump_file, max_sample_docs=50)
        await parser.connect()
        ...
        # Field distribution should accurately count ALL documents
        field_dist = idx.stats.field_distribution
        assert field_dist["id"] == total_docs  # Present in all 200 docs
        assert field_dist["title"] == total_docs  # Present in all 200 docs
        assert field_dist["late_field"] == 50  # Only present in last 50 docs
