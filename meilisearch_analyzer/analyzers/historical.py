...
class HistoricalAnalyzer:
    ...
    def _compare_index(
        ...
        resolved_findings = [f for f in old_index.findings if f.id not in new_finding_ids]
        ...
        settings_diff = self._diff_settings(old_settings, new_settings) if settings_changed else {}
        ...
        return IndexChange(
            ...
            finding_count=MetricChange.calculate(
                "finding_count", old_finding_count, new_finding_count, higher_is_better=False
            ),
    ...
    def _identify_trend_areas(
        ...
        if health_score.trend == TrendDirection.UP:
            ...
            improvements.append(
                f"Health score improved by {change_pct:.1f}%" if change_pct else "Health score improved"
            )
        elif health_score.trend == TrendDirection.DOWN:
            change_pct = health_score.change_percent
            degradations.append(
                f"Health score decreased by {abs(change_pct):.1f}%" if change_pct else "Health score decreased"
            )

        # Critical issues (lower is better)
        if critical_issues.trend == TrendDirection.DOWN and critical_issues.change < 0:
            improvements.append(f"Resolved {abs(int(critical_issues.change))} critical issue(s)")
        elif critical_issues.trend == TrendDirection.UP and critical_issues.change > 0:
            degradations.append(f"Added {int(critical_issues.change)} new critical issue(s)")
    ...
    def _generate_recommendations(
        ...
        new_critical = [
            fc for fc in finding_changes
            if fc.change_type == ChangeType.ADDED and fc.finding.severity.value == "critical"
        ]
        if new_critical:
        ...
        degraded_indexes = [
            uid for uid, change in index_changes.items()
