...
class BestPracticesAnalyzer(BaseAnalyzer):
                ...
                findings.append(
                    Finding(
                        ...
                        current_value={
                            ...
                            "settings_changed": sorted(setting_types) if setting_types else None,
                        },
                        references=[
    ...
    def _check_missing_embedders(
        ...
        for index in indexes:
            ...
            text_indicators = ["content", "body", "text", "description", "article", "post"]
    ...
    def _check_old_version(self, instance_version: str | None) -> list[Finding]:
        ...
        try:
            ...
            if current < stable:
                ...
                major_diff = int(stable_parts[0]) - int(current_parts[0]) if len(current_parts) > 0 and len(stable_parts) > 0 else 0
                minor_diff = int(stable_parts[1]) - int(current_parts[1]) if len(current_parts) > 1 and len(stable_parts) > 1 else 0

                # Only suggest if at least one minor version behind
                if major_diff > 0 or minor_diff >= 1:
                    severity = FindingSeverity.WARNING if major_diff > 0 else FindingSeverity.SUGGESTION
