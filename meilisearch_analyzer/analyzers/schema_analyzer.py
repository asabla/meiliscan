    ...
    def _check_searchable_attributes(self, index: IndexData) -> list[Finding]:
        ...
        if searchable == ["*"]:
            ...
            suggested_fields = [
                ...
                if not self._is_id_field(field) and not self._is_likely_numeric_only(field, index)
            ...
            findings.append(
                Finding(
                    ...
                    fix=FindingFix(
                        ...
                        payload={"searchableAttributes": suggested_fields} if suggested_fields else {},
                    ),
                    references=[
            ...
            if id_fields_in_searchable:
                recommended = [f for f in searchable if f not in id_fields_in_searchable]
                findings.append(
                    Finding(
                        id="MEILI-S002",
                        category=FindingCategory.SCHEMA,
            ...
            numeric_fields = [
                f for f in searchable if self._is_likely_numeric_only(f, index) and f not in id_fields_in_searchable
            ]
            if numeric_fields:
                findings.append(
                    Finding(
