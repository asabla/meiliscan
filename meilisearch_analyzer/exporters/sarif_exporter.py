...
# Tool info
TOOL_NAME = "meilisearch-analyzer"
TOOL_INFO_URI = "https://github.com/meilisearch/meilisearch-analyzer"
TOOL_VERSION = "0.1.0"
...
class SarifExporter(BaseExporter):
    ...
    def _build_results(self, findings: list[Finding]) -> list[dict[str, Any]]:
        ...
        for finding in findings:
            ...
            if finding.current_value is not None:
                result["properties"]["currentValue"] = self._safe_value(finding.current_value)
            if finding.recommended_value is not None:
                result["properties"]["recommendedValue"] = self._safe_value(finding.recommended_value)
    ...
    def _build_invocation(self, report: AnalysisReport) -> dict[str, Any]:
        ...
        has_critical = any(
            ...
            for f in self._collect_all_findings(report)
        )

        invocation: dict[str, Any] = {
            "executionSuccessful": True,
            "endTimeUtc": report.generated_at.isoformat() + "Z" if report.generated_at else None,
        }
                    ...
                    "replacements": [
                        {
                            ...
                            "insertedContent": {
                                "text": orjson.dumps(finding.fix.payload).decode("utf-8")
                            },
                        }
                    ],
                }
