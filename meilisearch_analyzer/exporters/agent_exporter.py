...
class AgentExporter(BaseExporter):
    ...
    def __init__(self, include_all_findings: bool = True, max_findings: int | None = None):
    ...
    def export(self, report: AnalysisReport, output_path: Path | None = None) -> str:
        ...
        if critical:
            lines.extend(self._build_issues_section(
        ...
        # Suggestions section
        suggestions = [f for f in all_findings if f.severity == FindingSeverity.SUGGESTION]
        ...
        if fixable_findings:
            lines.extend(self._build_quick_fix_section(
                fixable_findings,
                report.source.url if report.source else None,
            ))
    ...
    def _build_summary(self, report: AnalysisReport) -> list[str]:
        ...
        if summary.health_score is not None:
            ...
            lines.append(f"- **Health Score:** {summary.health_score}/100 ({health_status})")
    ...
    def _build_index_overview(self, report: AnalysisReport) -> list[str]:
        ...
        for index_uid, index_data in report.indexes.items():
            ...
            lines.append(f"| `{index_uid}` | {doc_count} | {finding_count} | {top_issue} |")
